...
const ProviderSignin = ({ isMobile, clientId, clientSecret, redirectUrl, }) => {
    const uri = isMobile
    ...
    const googleUrl = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${clientId}&redirect_uri=${uri}&scope=openid%20email%20profile&response_type=code&access_type=offline&state=${encodeURIComponent(`${redirectUrl}`)}`;
    return { redirectURL: googleUrl };
};
// const ProviderCallback = () => {};
const ProviderCallback = async ({ code, clientId, clientSecret, isMobile, }) => {
    try {
        const uri = isMobile
            ? `${process.env.NEXT_PUBLIC_FULLAUTH_URL}/api/auth/callback/mobile/google`
            : `${process.env.NEXT_PUBLIC_FULLAUTH_URL}/api/auth/callback/google`;
